...
public static class ResManager {
    ...
    public static void Read (string filePath) {
        ...
        foreach (var (resId, resource) in resFile.ResourceEntries) {
            if (resId == 0) {
                Console.WriteLine($"Warning ({(global::System.IO.Path.GetFileName(filePath))}): Skipping resource with id 0.");
                continue;
            }
            ...
            if (resource.info.Flags.HasFlag(ResourceFile.ResourceFlags.LZW)) {
                Console.WriteLine($"Warning ({(global::System.IO.Path.GetFileName(filePath))}/{resId:X4}): LZW Packing unsupported. Will write output as unpacked.");
            ...
            if (resourceRecord.TryGetValue(resId, out var prevResource)) {
                if (resource.info.ContentType != prevResource.Info.ContentType)
                    Console.WriteLine($"Warning ({(global::System.IO.Path.GetFileName(filePath))}/{resId:X4}): Content types do not match old: {prevResource.Info.ContentType} new: {resource.info.ContentType }. Will use new content type.");
                ...
                for (var chunkEntryIndex = 0; chunkEntryIndex < chunkData.Length; ++chunkEntryIndex) {
                    ...
                    var prevEntryData = prevResource.ChunkData[chunkEntryIndex];

                    if (newEntryData.Length == 0) { // Don't override
                        chunkData[chunkEntryIndex] = prevEntryData; // Use old data
                        continue;
                    }

                    Console.WriteLine($"{(global::System.IO.Path.GetFileName(filePath))}: Patching {resId:X4}/{chunkEntryIndex} type: {resource.info.ContentType}");
                }
            } else {
                Console.WriteLine($"{(global::System.IO.Path.GetFileName(filePath))}: Adding {resId:X4} type: {resource.info.ContentType}");
            }
